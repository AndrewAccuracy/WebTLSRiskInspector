...
实验部分构建了本地弱安全配置演示环境，并围绕基础扫描、扩大端口范围、无服务基线扫描和 HTTPS 自签名证书检测开展四组验证。结果表明，系统能够识别 8080 测试端口暴露、目录索引暴露、Server 版本信息泄露、多项安全响应头缺失以及证书不受信任等问题，典型场景下综合风险评分由 76 分上升至 88 分，风险等级由中风险提升为高风险。本文还将系统与 Nmap、Nessus、OWASP ZAP、nikto 等工具进行对比，以说明其在轻量化实现、低依赖运行和可解释输出方面的定位。为便于复现，项目代码、实验输出与报告材料已整理并推送至 GitHub 仓库 \href{https://github.com/AndrewAccuracy/WebTLSRiskInspector}{AndrewAccuracy/WebTLSRiskInspector}。
...
该函数实现了算法 \ref{alg:risk-score} 中描述的评分逻辑。等级判定采用“总分阈值与高风险项数量共同约束”的组合策略：
\begin{itemize}
...
表 \ref{tab:tool-compare} 表明，本系统并不追求与 Nessus 等工业级产品在规则覆盖面上的竞争，而是在以下几个维度上形成差异化定位：
