...
{\CJKfamily{zhkai}
随着网络服务部署频率的提升，开放测试端口、目录索引暴露、HTTP 安全响应头缺失、服务版本信息泄露和 HTTPS 配置不规范等基础安全问题，逐渐成为攻击者开展信息收集和后续利用的重要入口。针对这类高频且易被忽视的配置风险，本文设计并实现了一套本地网络资产与 Web 安全配置风险检测系统。系统以 Python 标准库为主要实现基础，围绕“端口探测、协议分析、规则评分、报告生成”构建检测流程，能够对目标主机开放端口、HTTP 安全头、目录浏览暴露、服务版本信息以及 TLS 证书可信性进行自动化检测，并输出结构化结果和风险报告。

实验部分构建了本地弱安全配置演示环境，并围绕基础扫描、扩大端口范围、无服务基线扫描和 HTTPS 自签名证书检测开展四组验证。结果表明，系统能够识别 8080 测试端口暴露、目录索引暴露、Server 版本信息泄露、多项安全响应头缺失以及证书不受信任等问题，典型场景下综合风险评分由 76 分上升至 88 分，风险等级由中风险提升为高风险。本文还将系统与 Nmap、Nessus、OWASP ZAP、nikto 等工具进行对比，以说明其在轻量化实现、低依赖运行和可解释输出方面的定位。本项目代码、实验输出与报告材料已整理并推送至 GitHub 仓库 \href{https://github.com/AndrewAccuracy/WebTLSRiskInspector}{AndrewAccuracy/WebTLSRiskInspector}。
...
许多真实安全事件并不是由高级攻击技术直接触发，而是以信息收集和低成本探测为起点。攻击者往往首先确认目标系统的开放端口、服务类型、目录结构、组件版本以及浏览器侧保护机制是否存在缺口 \cite{wstg,nessus-guide,nmap-book,shodan}。在这一过程中，目录索引暴露、测试端口遗留、默认页面未清理、证书过期、HTTP 安全头缺失等问题常常成为构造后续攻击链的突破口。这类风险虽然在单独出现时不一定立即造成严重后果，但一旦与组件漏洞、弱口令、权限配置错误或运维疏漏叠加，就可能迅速演化为数据泄露、横向移动或业务中断事件 \cite{nist-server,owasp-asvs,cis-controls,mitre-attck}.
...
本研究的意义主要体现在以下三个方面。第一，在方法层面，本文尝试将基础安全巡检任务拆解为端口探测、协议访问、规则映射和量化输出四个可编程阶段，展示如何将经验性的安全检查动作转化为程序逻辑 \cite{owasp-asvs,websec-book,practical-monitoring}。第二，在实现层面，本文以自研程序完成关键检测流程，避免将风险识别过程完全交给封闭工具，从而使端口状态、响应头特征、目录索引证据和评分规则都能够被逐步追踪。第三，在工程层面，该系统具备一定扩展潜力，可进一步演进为持续巡检、批量主机检测、风险台账生成或可视化安全基线工具的原型 \cite{nuclei,openvas,devsecops,security-automation}。
...
围绕上述目标，本文首先分析本地网络资产与 Web 基础配置中的典型风险点，在此基础上设计端口探测、HTTP/HTTPS 检测、规则评分与报告生成的系统架构，并使用 Python 标准库完成核心检测逻辑开发。为验证系统有效性，本文进一步构建弱安全配置演示环境，对典型风险进行复现实验，并结合实验结果讨论风险影响与预防改进措施。

\subsection{论文结构安排}

为便于系统性展示研究思路与实现过程，本文后续内容按如下结构展开：第二章对基础安全配置风险的典型表现、研究任务边界与系统需求进行分析；第三章给出系统总体设计思路、模块划分和风险评分模型；第四章介绍系统的关键实现方法与算法逻辑；第五章展示实验过程与执行结果；第六章对风险含义、量化结果和方法局限进行讨论；第七章补充算法设计与复杂度分析；第八章将本系统与主流安全扫描工具进行对比；第九章提出预防措施与改进建议；最后对全文工作进行总结。这样的结构安排能够将“问题提出一系统设计一实验验证一结果讨论”完整串联起来，使本文围绕同一检测对象形成闭环论证。

\section{相关问题分析与需求定义}

\subsection{基础安全配置风险的典型表现}

在网络与应用系统中，基础安全配置风险的外在表现非常丰富。按照检测对象和风险来源划分，可以大致归纳为“暴露面风险、协议配置风险、信息泄露风险和加密通信风险”四类 \cite{owasp-top10,wstg,nist-server,cis-benchmarks}。这四类问题之间既相互独立，又彼此联系，共同构成了系统在日常运行中的基础安全状态。
...
围绕上述研究任务，系统在设计时必须兼顾自主实现、结构化表达、风险解释和结果归档四个方面。也就是说，主要检测逻辑应由本人编写完成，输出结果应具备清晰的数据结构；风险输出不能只给出问题名称，还需要说明证据、分值和处置建议；检测结果还应能够支撑后续分析、复核与归档。
...
基于上述分析，本文将系统功能需求概括为目标发现、风险识别和结果表达三个层次。在目标发现层面，系统需要对指定主机的多个端口执行连通性检测，并对开放端口进行服务类型映射和基础暴露风险提示。在风险识别层面，系统需要对 HTTP 服务获取响应头和正文预览，检测目录索引暴露、版本信息泄露和安全响应头缺失等问题，同时对 HTTPS 服务执行证书与 TLS 基础安全检测。在结果表达层面，系统需要基于规则模型计算风险总分和总体等级，并自动生成 JSON 与 Markdown 报告文件。
...
除功能需求外，系统还需要满足轻量性、可解释性、可扩展性和可复现性等非功能要求。轻量性要求系统尽量依赖标准库，以降低部署和运行成本；可解释性要求每个风险项附带证据和整改建议；可扩展性要求后续能够较方便地增加新的检测规则与输出形式；可复现性则要求实验环境可以在本地快速搭建并重复执行。
...
图 \ref{fig:architecture} 展示了系统的总体架构，按照“输入层、端口探测层、协议分析层、风险评分层、报告输出层”的顺序组织，并辅以规则库、检测对象与证据链存储等支撑组件。该结构明确了系统模块边界与各模块之间的数据流关系。

\subsection{系统架构}

系统总体上划分为端口探测层、协议分析层、风险评分层和报告输出层。端口探测层通过 Socket 判断目标端口是否开放，协议分析层对开放服务发起 HTTP/HTTPS 请求并提取响应头、页面摘要、证书和 TLS 参数，风险评分层依据规则库生成风险项并计算综合分值，报告输出层则负责生成结构化 JSON 和可读性较强的 Markdown 文档。
...
结合具体实现，系统由参数解析、端口扫描、HTTP 风险检测、HTTPS 风险检测和风险汇总等模块共同组成。参数解析模块负责处理目标主机、端口集合、超时参数与输出路径，端口扫描模块基于 TCP 连接状态识别开放端口，HTTP 风险检测模块关注目录索引、版本暴露和安全头缺失问题，HTTPS 风险检测模块关注握手异常、证书时效和 TLS 协议安全性，风险汇总模块则负责分值统计、等级判定与报告生成。
...
算法 \ref{alg:risk-score} 描述了本文采用的风险评分与等级判定过程。该算法先对全部风险项进行累积分值统计，再结合高风险项和中风险项数量进行分层判断。这样可以避免单一总分掩盖高危问题：即使累计分值尚未达到最高阈值，只要目标系统存在若干高风险项，整体等级仍应相应上调。

\subsection{设计原则与关键权衡}

在系统设计过程中，本文对若干设计选择进行了权衡，并最终确定了适合本文目标的轻量化方案。首先，系统全程只使用 Python 标准库（\texttt{socket}、\texttt{ssl}、\texttt{http.client} 等），未引入任何第三方包。这一选择使程序能够在任何安装了 Python 3 的环境中直接运行，无需 \texttt{pip install}，同时也避免了检测逻辑被第三方库封装为难以解释的“黑盒”，从而更好体现自主编程的要求。相应的代价是部分高级功能（如异步扫描、TLS 指纹识别）需要自行实现，当前版本未完全覆盖。

其次，端口扫描采用同步串行方式，即逐个端口依次发起连接探测，而不是采用多线程或异步并发扫描。对于本文设定的本地检测范围，端口数量通常在个位数到十几个之间，串行方式的总耗时通常不超过几秒，能够满足验证需求，同时代码逻辑清晰、易于调试。若未来需要扫描数百个端口，可以将主循环替换为 \texttt{concurrent.futures.ThreadPoolExecutor}，而不必改变单个端口探测函数的核心逻辑，因此当前串行设计并不构成扩展障碍。

再次，风险识别逻辑完全由人工定义的规则驱动，而未采用机器学习方法。对于“响应头是否存在”“TLS 版本是否过旧”这类问题，其判定条件本质上是确定性的布尔规则，不存在需要模型拟合的概率分布，规则驱动方式反而更简洁、更可解释。与此同时，规则库可以通过修改字典随时扩展，无需重新训练模型，更适合轻量级原型系统的迭代需求。

最后，系统同时输出 JSON 和 Markdown 两种格式，而非只选其一。JSON 格式结构清晰、机器可读，适合后续脚本处理或接入可视化平台；Markdown 格式人类可读，便于纳入实验记录和风险台账。两种格式在代码层面共享同一份检测结果数据，额外开销极小，却显著提升了结果使用的灵活性。

\section{系统实现}

\subsection{实现环境}

系统以 Python 3 为开发语言，尽量使用标准库完成全部核心逻辑。其中，\texttt{socket} 用于端口连通性探测，\texttt{ssl} 用于 TLS 握手、证书信息与密码套件提取，\texttt{http.client} 用于构造 HTTP/HTTPS 请求，\texttt{argparse} 用于命令行参数解析，\texttt{json} 用于结构化结果导出，\texttt{dataclasses} 则用于封装结果对象。
...
端口风险规则库 \texttt{RISK\_PORTS} 将常见敏感端口映射为服务名称、风险提示和基础分值三元组。基础分值越高，表示该端口一旦开放所带来的暴露面风险越严重。例如 Redis（6379）直接暴露可能导致未授权访问和数据库清空，赋予最高基础分值 30；Telnet（23）明文传输远程管理凭据，赋予 25 分；FTP（21）和 MySQL（3306）各赋 20 分。443（HTTPS）与 8443（HTTPS-Alt）的基础分值均为 0，因为 HTTPS 端口本身并非暴露面风险，其安全性需要由 4.6 节的协议层检测模块进一步判定。
...
安全响应头规则库 \texttt{SECURITY\_HEADERS} 定义了需要检测的 HTTP 安全响应头名称与对应的风险分值。分值设计依据各响应头的防护价值：HSTS 和 CSP 对 HTTPS 强制与 XSS 防护作用最大，各赋 10 分；X-Frame-Options 防点击劫持赋 8 分；X-Content-Type-Options 防 MIME 嗅探赋 6 分；Referrer-Policy 防来源泄露赋 4 分。
...
数据类 \texttt{PortResult} 与 \texttt{Finding} 用于以结构化方式封装检测结果，二者均通过 Python 的 \texttt{dataclass} 装饰器定义。
...
端口扫描模块虽然采用串行实现，但其逻辑链路十分清晰，适合作为整个检测系统的入口。更重要的是，端口扫描结果并非最终输出，而是作为后续协议分析的筛选条件。只有当目标端口处于开放状态时，系统才继续进入 HTTP 或 HTTPS 检查阶段，因此该模块在整体流程中承担着“缩小分析范围、降低无效请求数量”的作用。
...
该函数的设计重点体现在统一接口、有限读取、异常处理和字段扩展四个方面。通过 \texttt{use\_https} 参数控制连接类型后，上层模块无需关心协议差异，只需传入不同标志即可复用同一套请求逻辑。正文读取上限设置为 512 字节（\texttt{resp.read(512)}），既足以匹配目录索引暴露的 \texttt{Index of /} 特征，也能避免下载超大页面导致的性能问题。若目标端口连接超时或拒绝连接，函数返回 \texttt{None} 而非继续向外抛出异常，调用方只需判断返回值即可决定是否跳过后续分析，从而保证扫描流程的健壮性。此外，HTTP 状态码（\texttt{\_status}）和正文预览（\texttt{\_preview}）以约定前缀存入同一字典，避免了额外定义数据类，也简化了跨模块的数据传递。

\subsection{HTTP 风险检测实现}

HTTP 风险检测模块首先使用 \texttt{HTTPConnection} 发起请求并获取响应头与页面预览，再根据规则判断是否存在安全头缺失、目录索引暴露和版本信息泄露等问题。具体而言，系统会读取 \texttt{Server} 头并判断其是否包含精确版本信息，随后遍历预设安全响应头集合以识别缺失项，最后对响应正文进行快速预览，匹配 \texttt{Index of /} 等目录暴露特征。
...
该函数实现了算法 \ref{alg:risk-score} 中描述的评分逻辑。等级判定采用“总分阈值与高风险项数量共同约束”的组合策略：当总分不低于 80 分，或高风险项不少于 3 个时，系统判定为高风险；当总分不低于 40 分，或存在至少 1 个高风险项，或中风险项不少于 4 个时，系统判定为中风险；其余情况则判定为低风险。

这一设计的合理性在于：即使总分尚未达到 80，只要存在若干高风险问题（如直接暴露的 Redis 或过期证书），整体安全状态同样需要被提高等级。因此，高风险项数量作为补充判定条件，可以降低单一总分模型对高危问题的稀释效应。
...
结构化输出与文档化输出并行是系统结果管理的重要设计。前者便于脚本继续处理，例如批量汇总多台主机结果、后续接入数据库或进行图表统计；后者便于归档复核和安全分析人员快速阅读。通过同时保留这两种输出形式，系统不仅能够证明程序已完成运行，也兼顾了结果可使用、可展示、可追溯的实践需求。
...
\texttt{main()} 函数体现了“流水线”式的程序设计思想。程序首先对每个端口调用 \texttt{scan\_port()}，得到 \texttt{PortResult} 列表；随后由 \texttt{calculate\_port\_risk()} 将开放端口转化为暴露面风险项，并根据端口类型分别调用 \texttt{check\_http\_service()} 或 \texttt{check\_https\_service()} 进行 Web 层深度检查；所有风险项收集完成后，程序按风险分值降序排列 \texttt{Finding}，再调用 \texttt{summarize()} 计算综合评分与等级；最后同时写出 JSON 和 Markdown 两种格式，并将 JSON 结果打印到终端供用户即时查看。
...
图 \ref{fig:directory-screenshot} 展示了本地弱配置演示服务的真实页面，可以看到浏览器列出了 \texttt{backup.zip}、\texttt{logs/}、\texttt{config/} 等目录内容；同时，扫描程序从 HTTP 响应头中捕获到 \texttt{Apache/2.4.49} 版本信息。这两项证据分别对应检测系统识别出的“目录索引暴露”与“Server 版本信息暴露”两个风险项。

\section{实验设计与执行结果}

\subsection{实验环境与整体流程}

实验以本地回环地址 \texttt{127.0.0.1} 为扫描目标。实验开始时先启动本地弱配置演示服务：
\begin{lstlisting}[language=bash]
python3 demo_vulnerable_server.py
\end{lstlisting}
随后调用主扫描程序对 80、443 和 8080 端口进行检测：
\begin{lstlisting}[language=bash]
...
图 \ref{fig:workflow} 描述了系统从输入与初始化、端口扫描、服务类型识别、协议检测与信息提取、规则匹配与风险识别到结果输出的完整工作流程，呈现了系统运行路径与各阶段的判定逻辑。
...
端口扫描结果显示，目标主机仅在 8080 端口上对外提供可访问服务，说明系统能够准确识别实验环境中的暴露入口。风险检测结果显示，系统并未止步于“端口开放”这一表层事实，而是进一步识别出目录索引暴露、版本信息泄露和多项安全响应头缺失等典型问题，说明系统已经具备了由连接性检测向配置性分析延伸的能力。
...
本系统输出的风险结果还具有一定的优先级指导意义。例如，在 8 个风险项中，“目录索引暴露”和“测试端口暴露”应优先整改，因为它们直接关系到暴露面的存在及敏感资源被枚举的可能性；“安全响应头缺失”虽更多属于浏览器侧防护不足问题，但若长期忽视，则会在系统面对复杂攻击链时进一步放大风险。也就是说，本文的结果不仅能说明“系统存在问题”，还能够支持“问题先后怎么处理”的基本决策。

仅展示程序能够运行并不足以说明系统具备安全分析价值。本文更强调通过输出结果解释系统状态、识别风险类别并形成整改优先级，使执行结果能够转化为可讨论、可复核、可处置的风险结论，而不只是编程功能的演示。
...
实验四需要先启动本地演示服务，使其同时监听 8080/HTTP 与 8443/HTTPS：
\begin{lstlisting}[language=bash]
python3 demo_vulnerable_server.py
\end{lstlisting}
随后运行风险扫描程序，并将 8443 纳入扫描端口范围：
\begin{lstlisting}[language=bash]
...
四组实验覆盖了低、中、高三个风险等级，验证了系统的评分模型能够随实际检测结果动态变化，不存在固定输出偏差。实验一与实验三的对比说明系统具有较好的检测精度；实验一与实验二的对比说明扩大扫描范围能够发现更多潜在风险；实验一与实验四的对比则进一步说明，在端口暴露面与 HTTP 配置问题不变的前提下，新增一个证书不受信任的 HTTPS 端口会使综合评分由 76 升至 88，并使风险等级由中风险升级为高风险。该变化与自签名证书在真实环境中通常需要较高处置优先级的风险特征一致。四组实验共同表明，4.6 节实现的 HTTP 与 HTTPS 检测模块均已在真实场景下产生过可验证的输出，系统的代码开发、执行结果与风险展示三个环节不存在未经验证的"空模块"。
...
在此基础上，实验四进一步引入了 HTTPS 证书可信性风险：当目标主机额外开放一个使用自签名证书的 HTTPS 端口（8443）时，系统新增"HTTPS 证书为自签名证书"风险项（中风险，12 分），综合评分由 76 分上升至 88 分，触发系统评分规则中"总分 $S \geq 80$"的条件，整体等级由中风险升级为\textbf{高风险}。这一变化直接体现了风险评分模型对新增风险项的敏感性，也说明 HTTPS 层面的配置问题即使不涉及协议版本和密码套件缺陷，仍可能显著改变系统整体风险等级。
...
本文系统可以视为一个面向安全状态识别的规则驱动算法框架。其核心不是依赖复杂机器学习模型，而是依赖“输入采集一特征提取一规则映射一风险量化”的处理链。对于本文场景而言，这种方法具有两个优势：第一，逻辑透明，便于解释每一步为什么这样做；第二，便于验证，能够清晰观察输入变化如何影响最终输出。
...
设待扫描端口数量为 $m$，每个开放 Web 端口所需执行的 HTTP 或 HTTPS 检查规则数量分别为 $h$ 和 $s$。系统整体时间开销主要来自端口扫描、协议检测和报告生成三个过程。端口扫描需要对每个端口执行一次连接尝试，时间复杂度可近似视为 $O(m)$；HTTP/HTTPS 检测需要对开放服务执行规则匹配，若开放端口数量为 $k$，则额外复杂度约为 $O(k(h+s))$；报告生成过程需要对全部风险项进行排序与输出，若风险项数量为 $n$，则复杂度约为 $O(n \log n)$。
...
在网络安全领域，已存在若干成熟的扫描与检测工具，它们从不同维度解决安全评估问题。本文在设计与实现过程中，对 Nmap、Nessus、OWASP ZAP 和 nikto 等典型工具的功能范围、使用方式与局限性进行了分析，以明确本系统的定位与差异。
...
Nessus 是商业化的综合漏洞扫描平台，规则库涵盖数万条漏洞检测项，支持 CVSS 评分与合规报告输出 \cite{nessus-guide}。其优势在于覆盖面广、报告专业，但也存在规则封闭、部署成本较高和商业授权依赖等局限。对于本文关注的可解释实现目标而言，过于完善的工具反而会掩盖检测原理，不利于展示风险识别的程序化过程。
...
表 \ref{tab:tool-compare} 表明，本系统并不追求与 Nessus 等工业级产品在规则覆盖面上的竞争，而是强调端到端透明性、综合评分能力、低运行门槛和面向复核报告的设计。从端口探测到风险评分，系统全部逻辑均由 Python 标准库实现，使用者可以直接阅读和修改每一条规则，理解每一项风险是如何被发现和评分的，这是 Nessus 等商业工具难以提供的。与 Nmap 和 nikto 主要提供原始扫描结果不同，本系统能够将多维度风险汇总为统一评分与等级，为使用者提供“当前安全状态有多严重”的直接判断。由于依赖 Python 标准库，系统可以在任何已安装 Python 3 的环境中直接运行，无需额外安装依赖包、数据库或代理服务，这在轻量化验证和快速原型场景中具有明显优势。同时，系统的 Markdown 报告输出与 JSON 结构化导出围绕“代码可读、结果可展示、分析可解释”的目标设计，也更适合课程作业和基础安全巡检中的复核需求。
...
端口暴露是本次实验中最先被系统识别出的风险入口。对于 8080 这类常用于测试、管理或备用 Web 服务的端口，首先应确认其业务必要性。如果该端口只是用于本地调试或临时验证，应在验证结束后关闭服务进程，并从防火墙规则、反向代理配置和启动脚本中移除对应暴露入口。如果该端口确有长期运行需求，则应限制访问来源，例如仅允许内网网段、VPN 出口或堡垒机访问，避免将管理型或测试型服务直接暴露在不可控网络范围内。
...
本文系统仍有进一步完善空间。后续可以引入并发扫描机制，提高多端口、多目标场景下的检测效率；也可以增加端口用途白名单，将“未知开放端口”与“允许开放端口”区分处理，从而减少人工研判成本。在风险规则方面，系统可继续扩展敏感路径检测规则，对备份文件、日志文件、配置文件和默认页面进行更细粒度识别，并增加漏洞情报关联能力，将服务版本与 CVE、CVSS、公开修复建议建立映射。在评分与展示方面，后续可引入资产重要度、暴露范围、证据强度和历史复发次数等权重，并增加 HTML 或 PDF 报告输出，使风险证据、图表和整改建议能够更直观地归档展示。此外，系统还可以增加基线对比功能，对两次扫描之间新增、消失或等级变化的风险项进行差异分析。对于 HTTPS 检测部分，后续应进一步完善证书有效期检查在非校验握手场景下的覆盖能力，例如在检测到自签名证书后，基于 \texttt{getpeercert(binary\_form=True)} 返回的原始证书数据解析有效期字段，使“证书可信性”与“证书有效期”两类判定在自签名场景下也能同时给出结论。
...
\bibitem{owasp-top10}
OWASP Foundation. \textit{OWASP Top 10: The Ten Most Critical Web Application Security Risks}. OWASP Project. URL: \url{https://owasp.org/www-project-top-ten/}.

\bibitem{rfc6797}
Hodges J, Jackson C, Barth A. \textit{HTTP Strict Transport Security (HSTS)}. RFC 6797. URL: \url{https://www.rfc-editor.org/rfc/rfc6797}.

\bibitem{csp}
West M, et al. \textit{Content Security Policy Level 3}. W3C Working Draft. URL: \url{https://www.w3.org/TR/CSP3/}.

\bibitem{rfc9110}
Fielding R, Nottingham M, Reschke J. \textit{HTTP Semantics}. RFC 9110. URL: \url{https://www.rfc-editor.org/rfc/rfc9110}.

\bibitem{rfc8446}
Rescorla E. \textit{The Transport Layer Security (TLS) Protocol Version 1.3}. RFC 8446. URL: \url{https://www.rfc-editor.org/rfc/rfc8446}.

\bibitem{nist}
Scarfone K, Jansen W, Tracy M. \textit{Guide to General Server Security}. NIST Special Publication 800-123. URL: \url{https://csrc.nist.gov/publications/detail/sp/800-123/final}.

\bibitem{nist-vuln}
Scarfone K, Souppaya M, Cody A, Orebaugh A. \textit{Technical Guide to Information Security Testing and Assessment}. NIST Special Publication 800-115. URL: \url{https://csrc.nist.gov/publications/detail/sp/800-115/final}.

\bibitem{nist-csf}
National Institute of Standards and Technology. \textit{Framework for Improving Critical Infrastructure Cybersecurity}. NIST Cybersecurity Framework. URL: \url{https://www.nist.gov/cyberframework}.

\bibitem{wstg}
OWASP Foundation. \textit{Web Security Testing Guide}. OWASP WSTG. URL: \url{https://owasp.org/www-project-web-security-testing-guide/}.

\bibitem{nessus-guide}
Tenable. \textit{Vulnerability Management Best Practices Guide}. Tenable Documentation. URL: \url{https://www.tenable.com/cybersecurity-guide/principles/vulnerability-management}.

\bibitem{nmap-book}
Lyon G F. \textit{Nmap Network Scanning}. Insecure.Com LLC. URL: \url{https://nmap.org/book/}.

\bibitem{shodan}
Matherly J. \textit{Shodan: The Search Engine for the Internet of Things}. Shodan Documentation. URL: \url{https://www.shodan.io/}.

\bibitem{nist-server}
Kissel R, Stine K, Scholl M, Rossman H, Fahlsing J, Gulick J. \textit{Security Considerations in the System Development Life Cycle}. NIST Special Publication 800-64 Rev. 2. URL: \url{https://csrc.nist.gov/publications/detail/sp/800-64/rev-2/final}.

\bibitem{owasp-asvs}
OWASP Foundation. \textit{Application Security Verification Standard}. OWASP ASVS. URL: \url{https://owasp.org/www-project-application-security-verification-standard/}.

\bibitem{cis-controls}
Center for Internet Security. \textit{CIS Critical Security Controls}. CIS. URL: \url{https://www.cisecurity.org/controls}.

\bibitem{mitre-attck}
MITRE Corporation. \textit{MITRE ATT\&CK Knowledge Base}. MITRE. URL: \url{https://attack.mitre.org/}.

\bibitem{devsecops}
OWASP Foundation. \textit{DevSecOps Guideline}. OWASP Project. URL: \url{https://owasp.org/www-project-devsecops-guideline/}.

\bibitem{ssdlc}
Microsoft. \textit{Security Development Lifecycle}. Microsoft SDL Documentation. URL: \url{https://www.microsoft.com/en-us/securityengineering/sdl}.

\bibitem{secure-by-design}
CISA. \textit{Shifting the Balance of Cybersecurity Risk: Principles and Approaches for Secure by Design Software}. CISA Guidance. URL: \url{https://www.cisa.gov/resources-tools/resources/secure-by-design}.

\bibitem{nist-scrm}
National Institute of Standards and Technology. \textit{Cybersecurity Supply Chain Risk Management Practices for Systems and Organizations}. NIST SP 800-161 Rev. 1. URL: \url{https://csrc.nist.gov/publications/detail/sp/800-161/rev-1/final}.

\bibitem{owasp-cheatsheet}
OWASP Foundation. \textit{OWASP Cheat Sheet Series}. OWASP Project. URL: \url{https://cheatsheetseries.owasp.org/}.

\bibitem{nist-80053}
National Institute of Standards and Technology. \textit{Security and Privacy Controls for Information Systems and Organizations}. NIST SP 800-53 Rev. 5. URL: \url{https://csrc.nist.gov/publications/detail/sp/800-53/rev-5/final}.

\bibitem{cis-benchmarks}
Center for Internet Security. \textit{CIS Benchmarks}. CIS Benchmark Series. URL: \url{https://www.cisecurity.org/cis-benchmarks}.

\bibitem{ops-report}
Google Cloud. \textit{Operational Excellence}. Google Cloud Architecture Framework. URL: \url{https://cloud.google.com/architecture/framework/operational-excellence}.

\bibitem{websec-book}
Stuttard D, Pinto M. \textit{The Web Application Hacker's Handbook}. Wiley. URL: \url{https://portswigger.net/web-security/web-application-hackers-handbook}.

\bibitem{practical-monitoring}
Turnbull J. \textit{The Art of Monitoring}. James Turnbull. URL: \url{https://www.jamesturnbull.net/books/the-art-of-monitoring/}.

\bibitem{nuclei}
ProjectDiscovery. \textit{Nuclei Documentation}. ProjectDiscovery. URL: \url{https://docs.projectdiscovery.io/tools/nuclei/overview}.

\bibitem{openvas}
Greenbone Networks. \textit{OpenVAS and Greenbone Vulnerability Management Documentation}. Greenbone. URL: \url{https://greenbone.github.io/docs/latest/}.

\bibitem{security-automation}
O'Connor T. \textit{Security Automation with Python}. Technical Reference.

\bibitem{software-security}
McGraw G. \textit{Software Security: Building Security In}. Addison-Wesley. URL: \url{https://www.informit.com/store/software-security-building-security-in-9780321356703}.

\bibitem{secure-coding}
Seacord R C. \textit{Secure Coding in C and C++}. Addison-Wesley. URL: \url{https://www.informit.com/store/secure-coding-in-c-and-c-plus-plus-9780321822130}.

\bibitem{threat-modeling}
Shostack A. \textit{Threat Modeling: Designing for Security}. Wiley. URL: \url{https://shostack.org/books/threat-modeling-book}.

\bibitem{redis-security}
Redis. \textit{Redis Security Documentation}. Redis Official Docs. URL: \url{https://redis.io/docs/latest/operate/oss_and_stack/management/security/}.

\bibitem{mysql-security}
Oracle. \textit{MySQL Security Guidelines}. MySQL Reference Manual. URL: \url{https://dev.mysql.com/doc/refman/8.4/en/security.html}.

\bibitem{xcto}
Mozilla. \textit{X-Content-Type-Options}. MDN Web Docs. URL: \url{https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/X-Content-Type-Options}.

\bibitem{referrer-policy}
W3C. \textit{Referrer Policy}. W3C Recommendation. URL: \url{https://www.w3.org/TR/referrer-policy/}.

\bibitem{clickjacking}
Rydstedt G, Bursztein E, Boneh D, Jackson C. \textit{Busting Frame Busting: A Study of Clickjacking Vulnerabilities}. IEEE Symposium Paper. URL: \url{https://seclab.stanford.edu/websec/framebusting/framebust.pdf}.

\bibitem{tls-bcp}
Sheffer Y, Saint-Andre P, Fossati T. \textit{Recommendations for Secure Use of Transport Layer Security (TLS) and Datagram Transport Layer Security (DTLS)}. RFC 9325, BCP 195. URL: \url{https://www.rfc-editor.org/rfc/rfc9325}.

\bibitem{mozilla-tls}
Mozilla. \textit{Server Side TLS Guidelines}. Mozilla Wiki. URL: \url{https://wiki.mozilla.org/Security/Server_Side_TLS}.

\bibitem{owasp-tls}
OWASP Foundation. \textit{Transport Layer Security Cheat Sheet}. OWASP Cheat Sheet. URL: \url{https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Security_Cheat_Sheet.html}.

\bibitem{apache-security}
Apache Software Foundation. \textit{Apache HTTP Server Security Tips}. Apache Documentation. URL: \url{https://httpd.apache.org/docs/current/misc/security_tips.html}.

\bibitem{nginx-security}
NGINX. \textit{Security Controls and Hardening Guides}. NGINX Documentation. URL: \url{https://docs.nginx.com/nginx/admin-guide/security-controls/}.

\bibitem{nist-crypto}
National Institute of Standards and Technology. \textit{Recommendation for Key Management}. NIST SP 800-57 Part 1 Rev. 5. URL: \url{https://csrc.nist.gov/publications/detail/sp/800-57-part-1/rev-5/final}.

\bibitem{cert-guide}
CA/Browser Forum. \textit{Baseline Requirements for the Issuance and Management of Publicly-Trusted Certificates}. CA/B Forum. URL: \url{https://cabforum.org/working-groups/server/baseline-requirements/}.

\bibitem{software-engineering}
Sommerville I. \textit{Software Engineering}. Pearson. URL: \url{https://www.pearson.com/en-us/subject-catalog/p/software-engineering/P200000003638}.

\bibitem{monitoring-book}
Beyer B, Jones C, Petoff J, Murphy N R. \textit{Site Reliability Engineering}. O'Reilly Media. URL: \url{https://sre.google/sre-book/table-of-contents/}.

\bibitem{secure-design}
Saltzer J H, Schroeder M D. \textit{The Protection of Information in Computer Systems}. Proceedings Paper. URL: \url{https://web.mit.edu/Saltzer/www/publications/protection/}.

\bibitem{platform-engineering}
Hightower K, Burns B, Beda J. \textit{Kubernetes: Up and Running}. O'Reilly Media. URL: \url{https://www.oreilly.com/library/view/kubernetes-up-and/9781492046523/}.
